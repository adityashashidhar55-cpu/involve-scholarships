...
function matchScholarship(
  ...
  // 5. NO RULES ON FILE - the one case that was silently reading as "eligible".
  //
  // A record with an empty `criteria` array cannot fail step 2, cannot raise a
  // blocking action in step 3 and cannot fail step 4. It therefore fell
  // straight through to "eligible now" having been checked against nothing but
  // the pre-filter: nationality, study level and destination country.
  //
  // That inverted the whole tool. The LESS we knew about an award, the more
  // confidently it was recommended - so the awards reaching the top of the list
  // were exactly the ones nobody had read the rules of. It is how a 35-year-old
  // MBA candidate was told the Rhodes Scholarship was open to them: Rhodes
  // carries a hard age limit, we had simply not recorded it, and an unrecorded
  // rule cannot fail.
  //
  // "We have not checked this one" is a different statement from "you are
  // eligible for this one", and that difference is the entire value of the
  // page. So it is now said out loud, in its own bucket, and it fails closed.
